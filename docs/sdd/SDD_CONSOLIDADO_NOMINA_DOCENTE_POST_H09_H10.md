...
| H19 | En validacion read-only; CSV `docentes.csv` normalizado y regenerado contra Cloud SQL sin escrituras, con reporte nominal y SQL en ROLLBACK para `teachers.created_by`. |
...
- H19 Directorio Docentes: validacion read-only regenerada del CSV `docentes.csv`; se generaron mapping, reporte nominal y SQL en `ROLLBACK` para 29 candidatos de `teachers.created_by`, sin ejecutar escrituras.
